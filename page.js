...
var genericPageSetup =
{
    ...
    site_icon: `<svg class="site_icon" viewBox="0 0 64 64">
        <path class="bg" d="M 32,0 0,12 0,44 32,64 64,44 64,12 z" />
        <path class="p" d="m 2.39,15.89 v 26.91 l 4.92,3.03 V 34.86 L 29.03,47.30 V 29.48 Z m 4.92,8 15.96,8.48 v 5.82 L 7.31,29.37 Z" />
        <path class="e" d="M 32,1.79 5.33,12 32,24.97 58.67,12 52.84,9.77 32,19.38 26.40,16.80 47.27,7.64 41.96,5.60 21.07,14.34 16,12 36.87,3.65 Z" />
        <path class="w" d="m 61.61,15.89 -4.92,2.51 V 40.34 l -5.11,3.04 v -22.37 l -5.31,2.71 v 22.81 l -5.53,3.28 V 26.54 L 34.97,29.48 V 59.18 L 61.61,42.80 Z" />
    </svg>`,

    menu_icon: `<svg class="menu_icon" viewBox="0 0 128 64">
        <path d="M 0,14 h64 v8 H 0 z m 0,14 h64 v8 H 0 z m 0,14 h64 v8 H 0 z" />
        <text id="menu_icon_text" x="127" y="32"></text>
    </svg>`,
    ...
    init: function pg_init()
    {
        ...
        this.make_header();
        var that = this;
        window.addEventListener('scroll', () => that.scroll_handler(), false);
        setInterval(() => that.time_handler(), 200);
        var nav_event_handler = e => that.navigation_handler(e);
    ...
    open_menu: function pg_open_menu()
    {
        ...
        // Adjust the page padding to compensate for a scroll bar that disappears when the page freeze is applied
        const scroll_bar_width = window.innerWidth - document.documentElement.clientWidth;
        document.documentElement.style.setProperty('--extra-page-padding-right', scroll_bar_width + 'px');
        document.body.classList.add('open_menu_page_freeze');
    ...
    close_menu: function pg_close_menu()
    {
        ...
        document.body.classList.remove('open_menu_page_freeze');
        document.documentElement.style.removeProperty('--extra-page-padding-right');
    },
    ...
    {
        const modification_time = new Date(document.lastModified).toISOString();
        const info = this.this_page_url + ' (modified ' + modification_time +
    ...
    navigation_handler: function pg_navigation_handler(event)
    {
        ...
        else if (history.state.state == this.menu_closed_history_state)
        {
            ...
            setTimeout(() => that.scroll_and_hide_header(old_scroll));
        }
        ...
        if (pending_menu_action)
        {
            ...
            setTimeout(() => eval(pending_menu_action), 0);
        }
    },
    ...
    update_menu_button_text: function pg_update_menu_button_text()
    {
        const menu_icon_text_elem = document.getElementById('menu_icon_text');
        if (!menu_icon_text_elem) return;

        const menu_item_count = this.extra_menu_buttons.length
            - !this.update_pg_addr_menu_button_info.is_visible
            - !this.reset_pg_addr_menu_button_info.is_visible;

        menu_icon_text_elem.textContent = menu_item_count;
    },
    ...
    make_header: function pg_make_header()
    {
        ...
        const footer_elem = body_elem.appendChild(document.createElement('div'));
        ...
        const is_site_icon_style_supported = +this.style_version >= 3;
        const is_menu_icon_style_supported = +this.style_version >= 5;

        let hs = '<div class="header_bar">';
        hs += '<a class="hdrhomebut" href="' + this.rel_path_to_root + 'index.html">'
            + (is_site_icon_style_supported ? this.site_icon : '')
            + ' PEW\'s Corner<\/a>';
        hs += '<button id="menubut" class="hdrnavbut">'
            + (is_menu_icon_style_supported ? this.menu_icon : 'Menu')
            + '<\/button>';
        hs += '<\/div>';
        ...
        const menu_container_elem = this.menu_group_elem.appendChild(document.createElement('div'));
        menu_container_elem.className = 'menu_container';

        const menu = menu_container_elem.appendChild(document.createElement('div'));
        menu.id = menu.className = 'menu';

        const that = this;
        const menu_item_click_handler = e => that.menu_item_click_handler(e);
        for (let i = 0; i < this.extra_menu_buttons.length; i++)
        {
        ...
        document.getElementById('menubut').addEventListener('click', () => that.open_menu(), false);
        ...
        this.menu_group_elem.addEventListener('click', () => that.close_menu(), false);
        ...
        body_elem.addEventListener('keydown', e => that.menu_key_down_handler(e), /* useCapture */ true);
    ...
    make_table_of_contents: function pg_make_table_of_contents()
    {
        ...
        for (var i = 0; i < a_elems.length; i++)
        {
            ...
            var that = this;
            a_elem.addEventListener('click', () => that.scroll_and_hide_header(), false);
        }
...
// Do the following actions immediately, but hide them in an anonymous function to avoid polluting the global namespace
(function() {
    // Remove any existing link to a favicon from the page (in case the page still has an old-style hardcoded link)
    const old_icon_link_elems = document.querySelectorAll('link[rel="icon"][href$="favicon.ico"]');
    for (let i = 0; i < old_icon_link_elems.length; i++)
    {
        old_icon_link_elems[i].parentNode.removeChild(old_icon_link_elems[i]);
    }

    // Add link elements for favicons to the page head element
    const icons = [
        ...
        {rel: 'apple-touch-icon', type: undefined, size: 180},
    ];
    for (const icon of icons)
    {
        const link_elem = document.createElement('link');
        link_elem.rel = icon.rel;
        if (icon.type) link_elem.type = icon.type;
        link_elem.sizes = icon.size + 'x' + icon.size;
        link_elem.href = genericPageSetup.rel_path_to_root + 'favicon02-' + icon.size + 'x' + icon.size + '.png';
        document.head.appendChild(link_elem);
    }

    // Remove any existing link to the global stylesheet from the page (in case the page still has an old-style
    // hardcoded link)
    const old_style_link_elems = document.querySelectorAll('link[rel="stylesheet"][href$="style2.css"]');
    for (let i = 0; i < old_style_link_elems.length; i++)
    {
        old_style_link_elems[i].parentNode.removeChild(old_style_link_elems[i]);
    }

    // Add a link to the global stylesheet to the page head element
    const style_link_elem = document.createElement('link');
    style_link_elem.rel = 'stylesheet';
    style_link_elem.type = 'text/css';
    style_link_elem.href = genericPageSetup.rel_path_to_root + 'style2.css';
    document.head.appendChild(style_link_elem);
})();

window.addEventListener('load', () => genericPageSetup.init(), false);
